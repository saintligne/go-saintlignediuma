...
{{end}}
RUN \
  echo 'gstle --cache 512 init /genesis.json' > gstle.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.saintlignedium/keystore/ && cp /signer.json /root/.saintlignedium/keystore/' >> gstle.sh && \{{end}}
	echo $'exec gstle --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--miner.etherbase {{.Etherbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> gstle.sh

ENTRYPOINT ["/bin/sh", "gstle.sh"]
`
...
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.saintlignedium{{if .Ethashdir}}
      - {{.Ethashdir}}:/root/.stlehash{{end}}
    environment:
      - PORT={{.Port}}/tcp
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.saintlignedium"],
		ethashdir:  infos.volumes["/root/.stlehash"],
		port:       port,
